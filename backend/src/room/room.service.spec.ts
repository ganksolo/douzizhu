...
describe('RoomService (Phase 21.1 Unit Tests)', () => {
    ...
    describe('joinRoom', () => {
        it('should create room and add player if not exists', async () => {
            ...
            const result = await service.joinRoom('1001', { id: '1', nickname: 'P1', avatar: 'a' }, {} as any);

            expect(mockRedisClient.hset).toHaveBeenCalledWith('room:1001:meta', expect.any(Object));
            expect(mockRedisClient.hset).toHaveBeenCalledWith('room:1001:players', '1', expect.any(String));
        ...
        it('should throw if room is full', async () => {
            ...
            mockRedisClient.hlen.mockResolvedValue(3); // Full

            await expect(service.joinRoom('1001', { id: '4', nickname: 'P4', avatar: 'a' }, {} as any))
    ...
    describe('leaveRoom', () => {
        it('should destroy room if last player leaves', async () => {
            ...
            mockRedisClient.hgetall.mockResolvedValue({}); // Empty after leave

            await service.leaveRoom('1001', '1');

            expect(mockRedisClient.del).toHaveBeenCalledWith('room:1001:meta');
            expect(mockRedisClient.del).toHaveBeenCalledWith('room:1001:players');
        });
    ...
    describe('kickPlayer', () => {
        it('should kick player if owner', async () => {
            ...
            mockRedisClient.hdel.mockResolvedValue(1);
            mockRedisClient.hgetall.mockResolvedValue({
                '1': JSON.stringify({ userId: '1' })
            }); // Owner remains

            await service.kickPlayer('1001', '1', '2');

            expect(mockRedisClient.hdel).toHaveBeenCalledWith('room:1001:players', '2');
        });
    ...
    describe('toggleReady', () => {
        it('should update ready status', async () => {
            ...
            expect(mockRedisClient.hset).toHaveBeenCalledWith('room:1001:players', '1', expect.stringContaining('"ready":true'));
    ...
    describe('requestRematch', () => {
        it('should reset room and players', async () => {
            ...
            expect(mockRedisClient.hset).toHaveBeenCalledWith('room:1001:players', '1', expect.stringContaining('"ready":false'));
            expect(mockRedisClient.hset).toHaveBeenCalledWith('room:1001:players', '2', expect.stringContaining('"ready":false'));
